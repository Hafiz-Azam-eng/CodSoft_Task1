    ...
    <div className="flex flex-col h-screen items-center justify-center bg-gray-100">
      ...
      {cart.length === 0 ? (
        ...
        <div className="flex flex-col items-center w-4/5 bg-white mb-8">
          ...
          <div className="total font-bold text-xl border px-8 py-6">
            Total: ${cart.reduce((total, item) => total + item.price * item.quantity, 0)}
          </div>
          <div className="total font-bold text-xl">
            <Link to="/checkOut">
              <button className='bg-[#e94863] px-8 py-4 text-white' onClick={makePayment}>CheckOut</button>
            </Link>
            <Link to="/checkOut">
              <button className='bg-[#e94863] px-4 py-4 text-white ml-1' onClick={makePayment}>C</button>
            </Link>
          </div>
          </div>
        </div>
